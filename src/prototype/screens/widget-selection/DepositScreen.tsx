...
export const contentMap: ContentMapScreenMetadata = {
  id: 'widget-selection-deposit',
  routePath: '/deposit',
  label: 'Deposit',
  context: 'Extra tokens deposit',
  heading: 'How much are you putting in?',
  subhead: 'Deposit screen — add money to get the extra sweepstake tokens chosen on the slider.',
  order: 2,
  options: [{ code: 'DEPOSIT', label: 'Deposit', to: 'savings' }],
};
...
// iOS-style numeric keyboard, reproduced from Figma node I108:4218.
// Key positions lifted directly from the Figma absolute layout.
...
export const DepositScreen: React.FC = () => {
  ...
  const [amountStr, setAmountStr] = useState(String(extraTokens));
  ...
  const handleDeposit = () => {
    navigate('/', {
      ...
      state: {
        ...
        depositedTokens: numericAmount, // $1 = 1 token
      },
    });
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Chase logo — blue square with cross pattern (no expiring asset URL) */}
          <div
            style={{
              width: 24,
              height: 24,
